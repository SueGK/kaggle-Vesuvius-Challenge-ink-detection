...
class SmpUnetDecoder(UnetDecoder):
	...
	def forward(self, encoder):
		...
		d = self.center(head)       # [32, 512, 7, 7] **unet** center

		decoder = []
		for i, decoder_block in enumerate(self.blocks): # 5 blocks - unet decoder
			# print(i, d.shape, skip[i].shape if skip[i] is not None else 'none')
			# print(decoder_block.conv1[0])
			# print('')
			s = skip[i]              # [32, 256, 14, 14] ...
			d = decoder_block(d, s)  # decoder_block(d[32, 512, 7, 7], s[32, 256, 14, 14]) ==> [32, 256, 14, 14] ...
			decoder.append(d)

		last  = d # [32, 16, 224, 224]  input of the model/encoder is volume [32, 12, 224, 224])
		return last, decoder
        # decoder
        # 0: [32, 256, 14, 14]
        # 1: [32, 128, 28, 28]
...
class Net(nn.Module):
	...
	def forward(self, batch):
		...
		]  # list [torch.Size([32, 8, 224, 224]), [32, 8, 224, 224], [32, 8, 224, 224]]
		K = len(vv) # 3
		...
		# ----
		encoder = []
		x = self.encoder.conv1(x)                       # torch.Size([96, 64, 112, 112])
		x = self.encoder.bn1(x)   # 
		x = self.encoder.act1(x)   ; encoder.append(x)  # torch.Size([96, 64, 112, 112])
		x = F.avg_pool2d(x,kernel_size=2,stride=2)      # torch.Size([96, 64, 56, 56])
		x = self.encoder.layer1(x) ; encoder.append(x)
		x = self.encoder.layer2(x) ; encoder.append(x)  # torch.Size([96, 128, 28, 28])
		x = self.encoder.layer3(x) ; encoder.append(x)  # torch.Size([96, 256, 14, 14])
		x = self.encoder.layer4(x) ; encoder.append(x)  # torch.Size([96, 512, 7, 7])
		...
		for i in range(len(encoder)):
			...
			_, c, h, w = f.shape
                        # K=3, B=32, c=64, h/w=112 C=12,H=224
			...
			w = F.softmax(f, 1) # [32, 3, 64, 112, 112]
			e = (w * e).sum(1) # sum(1) ==> [32, 64, 112, 112]
			encoder[i] = e
        #encoder [torch.Size([32, 64, 112, 112]), torch.Size([32, 64, 56, 56]), torch.Size([32, 128, 28, 28]), torch.Size([32, 256, 14, 14]), torch.Size([32, 512, 7, 7])]
		# ---
...
def train_one(net, d_train, d_valid):
    ...
    for t, xy0 in enumerate(batch_iter):
        ...
        batch = { 'volume': volume, 'inklabels': inklabels } # [32, 12, 224, 224]
        ...
        with amp.autocast():
            ...
            labels = batch['inklabels']   # [32, 1, 224, 224]
            loss_all = criterion(pred_labels, labels) # tensor(0.6158, device='cuda:0')loss.
             # 0.6158079504966736
            ...
            train_loss = loss_all / (t+1)
        if (t % 10 == 0):
            wandb.log({"train_loss":train_loss})
 
    print("lr: {:.2E}".format(scheduler.get_last_lr()[0]), flush=True)
    print("BCE loss: {:.3f}".format(train_loss), flush=True)
    ...
    #---
    net.eval()
    batch_iter_valid = np.array_split(xy, np.ceil(len(xy)/32)) # ceil(27390/32) = 856
    loss_all_valid = 0.0
    for t, xy0 in enumerate(batch_iter_valid):
        ...
        with torch.no_grad():
            with torch.cuda.amp.autocast(enabled=True):
                ...
                loss_all_valid += criterion(pred_labels, valid_labels)
                k += output['ink'].data.cpu().numpy()
                
        val_loss = loss_all_valid.detach().item()
        val_loss = val_loss / (t+1)  
        if (t % 10 == 0):
        ...
        print(f'\r @infer_one(): {t} / {len(batch_iter)} : {time_to_str(time.time() - start_timer, "sec")}', end='', flush=True)
