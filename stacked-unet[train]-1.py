...
class CFG:
    ...
    crop_depth = 12 #5, 12
    in_chans = 32 #16
    infer_fragment_z =  [32-16, 32+16]  #[24, 40]
        ...
        for y1 in y1_list:
            for x1 in x1_list:
                ...
                if fragment_id == CFG.valid_id:
                    ...
                    if tmp.max() != 0: # only choose areas with label 1
...
class Net(nn.Module):
	...
	def forward(self, images):
		...
		B,C,H,W = v.shape
		vv = [
			v[:,i:i+CFG.crop_depth] for i in [0,2,3,]
		]
		K = len(vv)
		x = torch.cat(vv,0)
		#x = v
...
if CFG.is_ckpt == True:  
    ...
    checkpoint = torch.load('/root/autodl-tmp/VCInkDectection/working/outputs/vesuvius/vesuvius_2d_slide_exp002/vesuvius-models/Unet_fold1_best.pth')
...
for epoch in range(CFG.epochs):
    ...
    if update_best:
        ...
        torch.save({'model': model.state_dict(),
                    ...
                    'lr': lr},
                    CFG.model_dir + f'{CFG.model_name}_fold{fold}_slice{CFG.in_chans}_crop_depth{CFG.crop_depth}_best.pth')